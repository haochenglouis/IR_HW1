...
def tfidf(document_id, query_name):
    score = 0
...
for query_name in query_dict:
	for doc_id in range(num_documents):
		score = tfidf(doc_id,query_name)
		results_store[query_name].append([score,id2doc_name[doc_id]])

with open('results/tfidf_results.out', 'w') as outputfile:
...
print('running tf_idf algorithm using',seconds, 'seconds.')
